...
onMounted(() => {
  if (!chatStore.currentUser || !chatStore.currentUser.id) {
  console.error('用户信息异常，请重新登录');
  router.push('/login');
  return;
  }
  chatStore.search();
...
// 计算属性：过滤出 receiverId 为 0 的消息（响应式）
const filteredHistory = computed(() => {
  return chatStore.history.filter(msg => msg.receiverId === 0);
});
...
<template>
  ...
  <div class="chat-header">
    <div class="chat-title">
      技术讨论群
      <span class="online-info"><span class="online-count"> {{websocket.onlineUserList.length }}</span>人在线</span>
    </div>
  </div>
